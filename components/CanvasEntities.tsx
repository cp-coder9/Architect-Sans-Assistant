...
export const AutoDimensionEntity: React.FC<AutoDimensionEntityProps> = ({ wall, openings, planCenter }) => {
    ...
    // Determine which side to place dimensions based on plan center
    if (planCenter) {
        const wallCenter = scale(add(wall.start, wall.end), 0.5);
        const toCenter = sub(planCenter, wallCenter);
    ...
    const tier1Offset = 200; // Increased offset to place outside rooms
    const tier2Offset = 300; // Increased offset for overall dimensions
